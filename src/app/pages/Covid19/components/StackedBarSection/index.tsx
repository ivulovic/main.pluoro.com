...
import { renderTimestampDate } from '@utils/date/covid';
...
export default function StackedBarSection(props: StackedBarProps) {
          ...
          this.points
            ? // @ts-ignore
              this.points.map(function (point, i) {
                const legendSymbol =
                  "<svg width='16' height='16'>" + point.series.legendSymbol.element.outerHTML + '</svg>';
                let result = legendSymbol + ' ' + point.series.name + ': ' + formatNumber(point.y, false, 0) + '';
                // @ts-ignore
  ...
  return (
    <div>
      <HighchartsReact options={options} callback={getChart} highcharts={Highcharts} />
    </div>
  );
}
